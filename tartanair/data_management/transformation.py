...
def se2SE(se_data):
    # Input is a 6x1 vector of [x,y,z,rx,ry,rz]
    # Output is a 4x4 matrix
    result_mat = np.matrix(np.eye(4))
    result_mat[0:3,0:3] = so2SO(se_data[3:6])
    result_mat[0:3,3]   = np.array(se_data[0:3, np.newaxis])
    return result_mat
...
def pos_quats2ses(pos_quat):
    traj_poses   = pos_quats2SEs(pos_quat) # From xyz, xyzw format, to SE(3) format (camera in world).
    traj_matrix  = pose2motion(traj_poses) # From SE(3) format, to flattened-tranformation-matrix (1x12) format.
    traj_motions = SEs2ses(traj_matrix).astype(np.float32) # From flattened-tranformation-matrix (1x12) format, to relative motion (1x6) format.
