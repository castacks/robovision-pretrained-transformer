...
from unimatch.unimatch_downsampled_flow import UniMatch
...
def get_args_parser():
    ...
    parser.add_argument('--task', default='matching', choices=['flow', 'stereo', 'depth', 'matching'], type=str) # TODO I know this says matching, but I'm using code from matching, so this will get updated to be 'flow'
...
def main(args):
    ...
    while total_steps < args.num_steps:
        ...
        for i in range(1): #FIXME
            batch_dictionary = convert_tartanair_batch_to_flow(
                batch_example, downsample_size=8, device = 'cuda') #tartanairdataloader.load_sample()

            downsampled_flow_preds = model(batch_dictionary['img0'], batch_dictionary['img1'],
                                 attn_type=args.attn_type,
                                 attn_splits_list=args.attn_splits_list,
                                 corr_radius_list=args.corr_radius_list,
                                 prop_radius_list=args.prop_radius_list,
                                 num_reg_refine=args.num_reg_refine,
                                 task='matching', # TODO I know this says matching, but I'm using code from matching, so this will get updated to be 'flow'
                                 )
