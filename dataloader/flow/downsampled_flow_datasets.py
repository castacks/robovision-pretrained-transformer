...
def convert_tartanair_batch_to_flow(batch, downsample_size=8, device = 'cuda'):
    ...
    mask = torch.where(mask != 0.0, 0.0, 1.0) #1 is valid, 0 is not

    downsampled_mask = torch.div(torch.nn.functional.avg_pool2d(mask, kernel_size=downsample_size, stride=downsample_size), downsample_size)
    downsampled_flow = torch.div(torch.nn.functional.avg_pool2d(flow, kernel_size=downsample_size, stride=downsample_size), downsample_size)

    return_dictionary = dict(img0 = images_1_tensor, img1 = images_2_tensor, downsampled_flow=downsampled_flow, downsampled_mask=downsampled_mask)
