...
try:
    ...
    def torch_meshgrid(*args, indexing='xy'):
        if indexing == 'xy':
            ...
            raise Exception(
                f'Expect indexing to be either "xy" or "ij". Got {indexing}. ')
...
class SensorModel(object):
    ...
    def make_shape_struct_from_repr(shape_struct):
        if isinstance(shape_struct, dict):
            ...
            raise Exception(
                f'shape_struct must be a dict or ShapeStruct object. Get {type(shape_struct)}')
...
class CameraModel(SensorModel):
    ...
    def pixel_coordinates(self, shift=0.5, normalized=False, flatten=False):
        ...
        If normalized is True, then the pixel coordinates are normalized to [-1, 1].
        '''
        xx, yy = self.pixel_meshgrid(
            shift=shift, normalized=normalized, skip_out_wrap=True, flatten=flatten)
    ...
    def get_valid_mask(self, flatten=False, force_update=False):
        ...
        pixel_coordinates = self.pixel_coordinates(
            shift=0.5, normalized=False, flatten=True)
...
class Pinhole(CameraModel):
    def __init__(self, fx, fy, cx, cy, shape_struct, in_to_tensor=False, out_to_numpy=False):
        ...
        super().__init__('Pinhole', fx, fy, cx, cy, fov_degree, shape_struct,
    ...
    def set_members_by_shape_struct(self, shape_struct):
        ...
        self.fov_degree_longitude = 2 * \
            math.atan2(shape_struct.W, 2 * self.fx) * 180.0 / LOCAL_PI
        self.fov_degree_latitude = 2 * \
            math.atan2(shape_struct.H, 2 * self.fy) * 180.0 / LOCAL_PI
...
# , clipping = 1000.
def flow_from_depth_motion(depth0, motion01, camera_model, device='cuda'):
    '''    
    ...
    # points can be n x 3 x h x w or 3 x h x w
    points0, G = camera_model.depth_to_points(depth0)

    # # Get the points in the new image frame.
    R1Inv = torch.tensor(Rotation.from_rotvec(
        motion01[:, 3:]).as_matrix()).float()
    R1 = R1Inv.mT.float().to(device)
    t1 = - \
        R1 @ (torch.from_numpy(motion01[:, :3]
                               ).unsqueeze(-1).float().to(device))

    # The coordinates in the world frame.
    # Coordinates in the NED frame. z-axis pointing downwards.
    XWorld_0 = points0.view(batchsize, 3, -1).roll(1, dims=1)
    ...
    # G1 is a 2xH*W tensor.
    G1, valid_mask1 = camera_model.point_3d_2_pixel(points1)
...
pin_depth0_gfps = [os.path.join(pin_depth_images_dir, f) for f in os.listdir(
    pin_depth_images_dir) if f.endswith('.png')]
...
pin_camera_model = Pinhole(fx=320, fy=320, cx=320, cy=320,
                           shape_struct=ShapeStruct(640, 640), out_to_numpy=False)
...
def build_train_dataset(batch_size=12, dataset_size=0.5, crop_size=[1/4, 1/4], downsample_size=8, standard_deviation=2):
    ...
    for ix in range(round(len(pin_depth0_gfps)*dataset_size-2)):
        ...
        random_crop_query_location = torch.rand(
            2).to('cuda')  # random crop location
        random_samples_reference_matrix = torch.rand(round(h * crop_size[0] / downsample_size) * round(
            w * crop_size[1] / downsample_size), 2).to('cuda')  # [Crop Area, 2]
        ...
        mask = torch.tensor(
            mask, device='cuda').float().unsqueeze(dim=-1) / 100

        # Masked pixels become 0
        masked_flow = torch.mul(flow, mask).permute(2, 0, 1)

        feature_flow = torch.nn.functional.avg_pool2d(
            masked_flow, kernel_size=downsample_size, stride=downsample_size)
...
# image2 = TartanAirImageReader().read_bgr(pin_bgr0_gfps[1])
image = torch.tensor(image, device='cuda').float().permute(2, 0, 1)
# print(image.shape)
# image2 = torch.tensor(image2, device='cuda').float()
# rgb_image = image.to('cpu').numpy().astype(np.uint8)
# rgb_image2 = image2.to('cpu').numpy().astype(np.uint8)
# cv2.imwrite('rgb-image2.png', rgb_image2)
crop_size = [1/4, 1/4]
downsample_size = 8
c, h, w = image.shape
...
random_samples_reference_matrix = torch.rand(
    feature_map_crop_height * feature_map_crop_width, 2).to('cuda')  # [Crop Area, 2]
...
mask = torch.where(mask != 0, 20, mask)
mask = torch.where(mask == 0, 1, mask)
mask = torch.where(mask == 20, 0, mask)

# print(mask.tolist())
# mask = torch.where(mask == 100, 255, mask)
# mask = torch.where(mask == 1, 255, mask)

# print(image.shape)
# mask = mask.unsqueeze(-1).repeat(1, 1, 3)

# grayscale_image = mask.to('cpu').numpy().astype(np.uint8)
# rgb_image = flow.to('cpu').numpy().astype()
# cv2.imwrite('grayscale-image.png', grayscale_image)

# overlayed_image = cv2.add(grayscale_image, rgb_image)

# cv2.imwrite('overlayed-image.png',overlayed_image)
...
flattened_feature_flow = feature_flow.view(c_ff, h_ff * w_ff)  # FIXME

# print(flattened_feature_flow[0, :])
# print(flattened_feature_flow.shape)

selective_masked_flow_indices = torch.round(
    random_samples_reference * h_ff * w_ff).to(torch.int32)
...
x_coords = ((selective_masked_flow_indices) %
            w_ff).unsqueeze(dim=-1).permute(1, 0)

y_coords = torch.floor(
    torch.div(selective_masked_flow_indices, w_ff)).unsqueeze(dim=-1).permute(1, 0)

####################################################################################################################

# Recovers x, y coordinates from 1D positions
reference_x_y_coords = torch.cat([x_coords, y_coords]).to(torch.int32) #good


# print(selective_masked_flow_indices[100])
# print(reference_x_y_coords[1, :])
# print(selective_masked_flow.shape)
# print(selective_masked_flow[0, :])
# print(flattened_feature_flow[0, :-6000])

# print(mask.shape)
# print(mask[:, :, 0].tolist())

correlation_positions = torch.add(selective_masked_flow, reference_x_y_coords).permute(1, 0) # good

correlation_samples, correlation_dimensions = correlation_positions.shape #good

correlation_gt_tensors = torch.zeros(size=(correlation_samples, feature_map_height, feature_map_width)).to('cuda')

#x_meshgrid, y_meshgrid = torch.meshgrid(correlation_gt_tensors)


# correlation_gt_tensors[correlation_positions.to('cpu').numpy()] = 255

# print(correlation_gt_tensors.shape)
# correlation_gt_tensor_image = correlation_gt_tensors[0, :, :].unsqueeze(dim=-1).to('cpu').numpy().astype(np.uint8)
# print(correlation_gt_tensor_image.shape)
# cv2.imwrite('correlation_gt_tensor_image.png',correlation_gt_tensor_image)


# print(correlation_gt_tensors[0, :, :].tolist())

# if(torch.any(correlation_gt_tensors == 1)):
#     print('1 is in the thing', torch.sum(correlation_gt_tensors == 1))
